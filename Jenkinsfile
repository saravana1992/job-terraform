...
node {
	...
	stage ('write back to customer git dir') {
		withCredentials([usernamePassword(credentialsId: 'git_new', usernameVariable: 'username', passwordVariable: 'password')]){
			sh ("git remote rm origin && git add . && git commit -m 'new terraform files' && git config --global push.default simple && git remote set-url origin https://$username:$password@github.com/saravana1992/test-repo.git && git push origin HEAD:master")
		}
	}
}
